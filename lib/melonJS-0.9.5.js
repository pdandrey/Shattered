...
(function($) {
	...
	Object.extend = function(prop) {
        ...
        var classname = "";
        if(prop.__classname) {
            classname = prop.__classname;
            delete prop.__classname;
        }
		...
		var Class = eval("(function " + classname + "() { if(!initializing && this.init) { this.init.apply(this, arguments); } })");
	...
	/** @scope me.SpriteObject.prototype */
	{
		...
		init : function(x, y, image, spritewidth, spriteheight) {
            ...
            function setOffset(value) {
                //console.log("Offset set to %o", value);
                this._offset = value;
            }

            Object.defineProperty(this, "offset", {get: getOffset, set: setOffset, enumerable: false});
				...
				for(var t=0, l=obj.touches.length; t<l; t++) {
					...
					for (var i = handlers.length, handler; i--, handler = handlers[i];) {
						...
						if (handler.floating===false) {
                            e.pos = {x: x + vpos.x - map_pos.x, y: y + vpos.y - map_pos.y };
						} else {
                            e.pos = {x: x, y: y};
						}
		...
		obj.registerMouseEvent = function(eventType, rect, callback, floating) {
			...
			switch (eventType) {
				...
				case 'touchend':
					...
					var _float = rect && rect.floating===true?true:false;
